...
void PolyaGammaD::rng_init(int static_seed, int seed){
...
PolyaGammaD::PolyaGammaD(int trunc) : T(trunc), bvec(T)
{
} // PolyaGammaD

void PolyaGammaD::insertEvent(cl_event* event,CLptrs *clptr){
...
double PolyaGammaD::calculateTime(cl_event* event1,cl_event* event2){
...
void PolyaGammaD::initCL(int n, int b, int static_seed, int seed,CLptrs *clptr){

  nums=n;
  batch=b;
  n=(int)(ceil(n/((double)b)));
  ...
  zbuf = clCreateBuffer( clptr->context, CL_MEM_READ_ONLY, nums*sizeof(double), NULL, &status);
  result = clCreateBuffer( clptr->context, CL_MEM_WRITE_ONLY, nums*sizeof(double), NULL, &status);
...
void PolyaGammaD::drawCL( double* x, int num, double* z, int local ,CLptrs *clptr){



  int n;
  n=(int)(ceil(nums/((double)batch)));
  for(int i=0;i<n;i++){
      seed_table[i].s[0]=(unsigned int)(unif_rand()*4294967295 );
      seed_table[i].s[1]=(unsigned int)(unif_rand()*4294967295 );
      seed_table[i].s[2]=(unsigned int)(unif_rand()*4294967295 );
  ...
  globalWorkSize[0] = (int)ceil(num/((double)batch));
  globalWorkSize[0] += (local-globalWorkSize[0]%local);
  status |= clEnqueueWriteBuffer( clptr->cmdQueue, zbuf, CL_TRUE, 0, num*sizeof(double), z, 0, NULL, NULL);
  //execute the kernel
  status=0;
  ...
  if(status!=0)
    Rprintf("draw: %d\n",status);
  status |= clEnqueueReadBuffer( clptr->cmdQueue, result, CL_TRUE, 0, nums*sizeof(double), x, 0, NULL, NULL);

}

void PolyaGammaD::drawCL( int local ,CLptrs *clptr){
...
//    test++;
  ...
  localWorkSize[0] = local;
  globalWorkSize[0] = (int)ceil(nums/((double)batch));
  globalWorkSize[0] += (local-globalWorkSize[0]%local);
...
void PolyaGammaD::step2CL(int j, int N, int P, CLptrs *clptr){
...
void PolyaGammaD::betaCL(int P, int N, int j, int J, int m, CLptrs *clptr){
  size_t globalWorkSize[1];
...
void PolyaGammaD::cholesky(double* b, double* R, double*d, int P){
  double *L;
  double *V;
  double *V1;
  ...
  lower=(double*)malloc(P*P*sizeof(double));
  ...
  free(mean);free(lower);
}
void PolyaGammaD::step1CL(int N, int J, int j,CLptrs *clptr){
...
void PolyaGammaD::initData(int nr,int nc,int P, int *nn, double *tX, double *z,double *P0, double *b0, double *XB,CLptrs *clptr){
  
  xb_no_j_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*(nc-1)*nr, NULL, &status);
  if(status!=0)
    Rprintf("create1: %d\n",status);
  minmax_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*nr, NULL, &status);
  if(status!=0)
    Rprintf("create2: %d\n",status);
  xb_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*nc*nr, NULL, &status);
  if(status!=0)
    Rprintf("create3: %d\n",status);
  c_j_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*nr, NULL, &status);
  if(status!=0)
    Rprintf("create4: %d\n",status);
  A_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*nr, NULL, &status);
  if(status!=0)
    Rprintf("create5: %d\n",status);
  eta_j_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*nr, NULL, &status);
  if(status!=0)
    Rprintf("create6: %d\n",status);
  tX_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*nr, NULL, &status);
  if(status!=0)
    Rprintf("create7: %d\n",status);
  tXOmega_buffer = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*nr, NULL, &status);
  if(status!=0)
    Rprintf("create8: %d\n",status);
  w_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*nr*(nc-1), NULL, &status);
  if(status!=0)
    Rprintf("create9: %d\n",status);
  tXOmX_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*P*clptr->MMT, NULL, &status);
  if(status!=0)
    Rprintf("create10: %d\n",status);
  tXOmC_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*clptr->MVT, NULL, &status);
  if(status!=0)
    Rprintf("create11: %d\n",status);
  mean_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P, NULL, &status);
  if(status!=0)
    Rprintf("create12: %d\n",status);
  beta_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P, NULL, &status);
  if(status!=0)
    Rprintf("create13: %d\n",status);
  Z_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*(nc-1), NULL, &status);
  if(status!=0)
    Rprintf("create14: %d\n",status);
  P0_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*P*(nc-1), NULL, &status);
  if(status!=0)
    Rprintf("create15: %d\n",status);
  b0_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*(nc-1), NULL, &status);
  if(status!=0)
    Rprintf("create16: %d\n",status);
  L_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*P, NULL, &status);
  if(status!=0)
    Rprintf("create17: %d\n",status);
  V_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*P, NULL, &status);
  if(status!=0)
    Rprintf("create18: %d\n",status);
  V1_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*P, NULL, &status);
  if(status!=0)
    Rprintf("create19: %d\n",status);
  lower_buffer=clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, sizeof(double)*P*P, NULL, &status);
  if(status!=0)
    Rprintf("create20: %d\n",status);
  int *n_buf,*n1_buf;
  ...
  if(status!=0)
    Rprintf("write n1: %d\n",status);
  status=clEnqueueWriteBuffer( clptr->cmdQueue, tX_buffer, CL_TRUE, 0, sizeof(double)*P*nr, tX, 0, NULL, NULL);
  if(status!=0)
    Rprintf("write tX: %d\n",status);
  status=clEnqueueWriteBuffer( clptr->cmdQueue, xb_buffer, CL_TRUE, 0, nc*nr*sizeof(double), XB, 0, NULL, NULL);
  if(status!=0)
    Rprintf("write xb: %d\n",status);
  status=clEnqueueWriteBuffer( clptr->cmdQueue, P0_buffer, CL_TRUE, 0, sizeof(double)*P*P*(nc-1), P0, 0, NULL, NULL);
  if(status!=0)
    Rprintf("write p0: %d\n",status);
  
  status=clEnqueueWriteBuffer( clptr->cmdQueue, b0_buffer, CL_TRUE, 0, sizeof(double)*P*(nc-1), b0, 0, NULL, NULL);
  if(status!=0)
    Rprintf("write b0: %d\n",status);
  status=clEnqueueWriteBuffer( clptr->cmdQueue, Z_buffer, CL_TRUE, 0, sizeof(double)*P*(nc-1), z, 0, NULL, NULL);
  if(status!=0)
    Rprintf("write Z: %d\n",status);
  ...
  int nnn=(int)(ceil(nums/((double)batch)));
  ...
  if(status!=0)
    Rprintf("create22: %d\n",status);
  result = clCreateBuffer( clptr->context, CL_MEM_READ_WRITE, nums*sizeof(double), NULL, &status);
  if(status!=0)
    Rprintf("create23: %d\n",status);
...
void PolyaGammaD::copyCL(int N, int J, CLptrs *clptr){
...
void PolyaGammaD::freeData(CLptrs *clptr){
